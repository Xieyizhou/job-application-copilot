...
from dashboard_review import review_inbox_view_matches, sorted_review_jobs
...
def render_full_jd_recovery(
    ...
    *,
    ...
    if services.demo_mode_enabled():
        ...
        return False
    st.markdown("**Full job description required**")
    st.caption(
        f"Current quality: {quality.get('display_label', 'Needs review')}. "
        f"{quality.get('next_action', 'Add the complete responsibilities and requirements.')}"
    )
    if not services.jsearch_configured():
        st.caption(
            "Configure JSEARCH_API_KEY to find the full posting automatically, "
            "or replace this record with the complete JD."
        )
        return False
...
def filtered_review_jobs(
    ...
    for original in all_jobs:
        ...
        job["fetch_run_date"] = str(
            fetch_run.get("created_at", job.get("last_seen_at", ""))
        )
...
def render_review_cover_letter_section(
    ...
    default_company = str(
        latest_fields.get("company_normalized")
...
def render_selected_review_detail(
    ...
    if section != st.session_state["selected_review_tab"]:
        ...
        st.rerun()
    with st.container(height=420, border=False, key="review_detail_panel"):
        if section == "Overview":
            ...
            render_review_jd_section(
                job, selected_text, selected_path, selected_tracker_row, context["jd_quality"], services
            )
        else:
...
def job_descriptions_tab(services: ReviewPageServices) -> None:
    ...
    with left_panel, st.container(key="review_job_list_panel"):
        ...
        if services.demo_mode_enabled():
            st.caption(
                "Three fictional roles demonstrate strong, uncertain, and weak fit outcomes."
            )
        ...
        shortlist = filtered_jobs[: services.max_recommendation_limit]
        if not shortlist:
            render_empty_review_state(filters)
            return
        if len(filtered_jobs) > len(shortlist):
            st.caption(f"First {len(shortlist)} of {len(filtered_jobs)} matches")
